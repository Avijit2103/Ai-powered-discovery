...
function norm(t: string) { return t.toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim() }
function tokens(t: string) { return norm(t).split(' ').filter(Boolean) }
...
const BAGS: Bag = {
  ...
  Dental: [
    { k: 'tooth' }, { k: 'teeth' }, { k: 'toothache' }, { k: 'tooth pain', w: 2 }, { k: 'molar' }, { k: 'wisdom tooth', w: 2 },
...
function classifyLocal(input: string): Category {
  const text = norm(input)
  const T = tokens(text)
  if (text.length < 8 || T.length < 2) return 'Unknown'
  const VAGUE = /\b(not feeling well|unwell|sick|health issue|health problem|pain|hurting|ill)\b/
  if (VAGUE.test(text)) {
    const hasSignal = /(tooth|teeth|dent|gum|cavity|root canal|rct|eye|vision|glass|specs|optici|optomet|ophthalm|therapy|counsel|psych|anx|depress|insomnia|doctor|clinic|opd|fever|cough|sore throat|x-?ray|ultrasound)/.test(text)
    if (!hasSignal) return 'Unknown'
  }
  if (/\b(tooth|teeth|toothache|tooth pain|dentist|dental|molar|gum)\b/.test(text)) return 'Dental'
  if (/\b(glasses|spectacles|specs|eyewear|optician|optomet)\b/.test(text)) return 'Vision'
  if (/\b(therapy|therapist|counsel|psych|anxiety|depress|insomnia|stress)\b/.test(text)) return 'Mental Health'
  if (/\b(opd|doctor|clinic|checkup|fever|cough|cold|prescription|x-?ray|ultrasound)\b/.test(text)) return 'OPD'
  const cats = Object.keys(BAGS) as Array<keyof typeof BAGS>
  const scores = cats.map(c => [c, scoreBag(text, BAGS[c])] as const).sort((a, b) => b[1] - a[1])
  const [bestCat, bestScore] = scores[0]
  const secondScore = scores[1]?.[1] ?? 0
  const strongCats = scores.filter(([_, s]) => s >= 1.8).length
  if (strongCats >= 2) return 'Unknown'
  if (bestScore < 1.5) return 'Unknown'
  if (bestScore - secondScore < 0.8) return 'Unknown'
...
const PLAN_TEMPLATES: Record<Exclude<Category, 'Unknown'>, string[][]> = {
  Dental: [
    ...
    [
      ...
      'Get the treatment plan approved if cost is high',
  ...
  Vision: [
    [
      ...
      'Complete the eye test and upload the prescription',
      'Claim frames or lenses within the annual limit via the portal'
    ],
    [
      'Find a network optical store in the portal',
  ...
  'Mental Health': [
    ...
    [
      ...
      'Book a slot and confirm coverage before the session',
      'Join the session and save the visit summary'
    ]
  ],
  OPD: [
    [
      'Book a general physician from the portal',
      'Use cashless or keep bills if out-of-network',
      'Upload prescription and invoice for OPD reimbursement'
    ],
    [
      'Find a nearby network clinic/doctor',
      'Take your e-card and photo ID to the visit',
      'Submit consultation bill and Rx in the portal'
    ]
  ]
}
